      ...
      <div className="mx-auto max-w-2xl rounded-2xl border border-[#1C2431] bg-[#0D1117] p-8 shadow-2xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Register your ISP and submit your payment for verification.</p>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="grid gap-5 md:grid-cols-2">
          {[
            ...
            <div key={key} className={key === "company_name" ? "md:col-span-2" : ""}>
          ...
          {error && <div className="md:col-span-2 rounded-lg bg-red-500/10 p-3 text-sm text-red-400">{error}</div>}

          <button disabled={loading} className="md:col-span-2 flex h-12 items-center justify-center rounded-lg bg-blue-600 font-medium hover:bg-blue-700 disabled:opacity-50">
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Create Account & Continue to Payment"}
          </button>

          <p className="md:col-span-2 text-center text-sm text-slate-500">
            Already registered? <a href="/" className="text-blue-400 hover:text-blue-300">Sign in</a>
          </p>
        </form>
      </div>
    </main>
