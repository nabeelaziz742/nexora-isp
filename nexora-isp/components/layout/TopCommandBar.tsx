...
function resolvePageTitle(pathname: string) {
  if (pathname === "/customers/new") return "Customer Activation";
  ...
  return matchedRoute ? pageTitles[matchedRoute] : "NEXORA ISP";
}
    ...
    <header className="flex h-16 shrink-0 items-center border-b border-[#202938] bg-[#0D1117] px-3 sm:px-4 lg:px-5">
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-slate-100">{pageTitle}</p>
          <p className="hidden text-[10px] uppercase tracking-[0.08em] text-slate-600 sm:block">NEXORA ISP</p>
        </div>
      </div>
      ...
      <div className="hidden min-w-0 flex-1 justify-center xl:flex">
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate text-xs text-slate-600">Search customers, nodes, tickets...</span>
          <span className="border border-[#202938] bg-[#121821] px-1.5 py-0.5 font-mono text-[9px] text-slate-600">⌘ K</span>
        </button>
      </div>
        ...
        <div ref={profileRef} className="relative">
          ...
          >
            <div className="hidden min-w-0 text-right sm:block">
              <p className="max-w-28 truncate text-[11px] font-medium text-slate-200">{organization?.name ?? "NEXORA ISP"}</p>
              <p className="max-w-28 truncate text-[10px] text-slate-600">
                {fullName || user?.email || "User"}{role ? ` · ${role.toLowerCase()}` : ""}
              </p>
            </div>

            <div className="flex h-8 w-8 shrink-0 items-center justify-center bg-blue-600 text-[11px] font-semibold text-white">{initials}</div>
          ...
          {profileOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-56 border border-[#202938] bg-[#0D1117] shadow-2xl">
              <div className="border-b border-[#202938] px-4 py-3">
                <p className="truncate text-xs font-medium text-slate-200">{fullName || "NEXORA User"}</p>
